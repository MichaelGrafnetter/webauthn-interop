...
Before any other Okta-related PowerShell cmdlets can be used, you must first connect to your Okta tenant using the [Connect-Okta](PowerShell/Connect-Okta.md) cmdlet. The following authentication methods are supported:

- API token (legacy, not recommended)
- Authorization code flow with PKCE (interactive login, highly recommended)
- Client credentials flow (headless use)
  - Public key/private key (highly recommended)
  - Client secret (not recommended)

With the exception of the API token method, all of these authentication methods require you to create an application registration within your Okta tenant and to grant that application the appropriate permissions. The process for creating an application and granting permissions is slightly different for each authentication method.

The setup process for the recommended authentication methods is described in the sections below. You only need to complete the configuration for the authentication method(s) you intend to use.

> [!Note]
> See the [Set up Okta for OAuth API access](https://developer.okta.com/docs/guides/set-up-oauth-api/main/) article in the Okta developer documentation for more information on these authentication methods and their setup processes.
...
![Create API services app](Screenshots/okta-register-app-api.png)

On the next page, give the app integration a meaningful name. Click save.

![API services app name](Screenshots/okta-register-app-api-name.png)
...
![Switch client authentication to public key/private key](Screenshots/okta-register-app-secret.png)
...
![Add public key](Screenshots/okta-register-app-add-key.png)
...
![Paste or generate public key dialog](Screenshots/okta-register-app-paste-key.png)
...
![Generated private key in JSON format](Screenshots/okta-register-app-generate-key.png)
